	...
	var fnUploadImage = null;
 	...
 	/* Custom */ 
	/** Csrf 
	 *  - callFileUploader() sUrl에 필요 */
	var csrfToken = $Element("csrf").$value().value;	
	var csrfHeader = $Element("csrf").$value().getAttribute("name");
	
	/** photo_upload.jsp -> #info */
	var elPhotoAttachGuide = jindo.$$.getSingle("#info",$("pop_content2"));
	var photoAttachGuide = $Element(elPhotoAttachGuide);
		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = false; //HTML5 다중업로드 사용여부
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
	 ...
	 */
     function goStartMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") < 0 ){
    		 welBtnConfirm.attr("src", baseUrl + "/resources/vendors/SE2.8.2.O12056/sample/photo_uploader/img/btn_confirm2.png");
     ...
     function goReadyMode(){
    	 ...
    	 if(sSrc.indexOf("btn_confirm2.png") >= 0 ){
	    	 ...
	    	 welBtnConfirm.attr("src", baseUrl + "/resources/vendors/SE2.8.2.O12056/sample/photo_uploader/img/btn_confirm.png");
    	 }
     }   
     ...
     */
 	...
 	/* Custom  
 	 * 
 	 * - Photo_uploader.jsp #info
 	 *  파일 첨부하면 숨기고 파일첨부 안하면 보여주는 기능
 	 */
 	
 	function hidePhotoAttachGuide () {
 		photoAttachGuide.hide();
 	}
 	
 	function showPhotoAttachGuide () {
 		photoAttachGuide.show();
 	}
    ...
    function html5Upload() {	
    	...
    	sUploadURL= location.protocol+'//'+location.host + '/board/multiplePhotoUpload?'+ csrfHeader + '=' + csrfToken ; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		oAjax.header(csrfHeader,csrfToken);
...
//     location.href.replace(/\/[^\/]*$/, '') + '/photoUpload'
...
// 	
 		...
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			...
 			sUrl  : location.protocol+'//'+location.host + '/' + location.pathname.split("/")[1] + '/board/photoUpload?'+ csrfHeader + '=' + csrfToken ,	// URL.
...
// 	    	    console.log('oCustomEvent.sValue : ' + oCustomEvent.sValue);
// 	    	    console.log('oCustomEvent.bAllowed : ' + oCustomEvent.bAllowed);
// 	    	    console.log('oCustomEvent.sMsgNotAllowedExt : ' + oCustomEvent.sMsgNotAllowedExt);
 	    	
//  				선택된 파일의 형식이 허용되는 경우만 처리 
 	    		if(oCustomEvent.bAllowed === true){
 		    		goStartMode();
 		    		hidePhotoAttachGuide();
 		    	}else{
 		    		goReadyMode();
 		    		showPhotoAttachGuide();
 		    		oFileUploader.reset();
 		    	}
...
// 	    		alert('sFileName :: ' + oCustomEvent.htResult.sFileName + '\n'
// 	    			 + 'sFileURL :: ' + oCustomEvent.htResult.sFileURL + '\n'
// 	    			 + 'bNewLine :: ' + oCustomEvent.htResult.bNewLine + '\n'
// 	    			 + 'sAlign :: ' + oCustomEvent.htResult.sAlign + '\n' 
// 	    			 + 'oOriginImgWidth :: ' + oCustomEvent.htResult.oOriginImgWidth + '\n');
